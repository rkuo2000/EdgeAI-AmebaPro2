...
class PortraitRig {
  constructor(image) {
    ...
    this.swayBuffer = document.createElement('canvas');
    this.swayBuffer.width = 320;
    this.swayBuffer.height = 480;
    this.swayCtx = this.swayBuffer.getContext('2d');
  ...
  armPose(now) {
    if (this.reduced.matches || (!this.speaking && !this.ending)) return [0,0];
    const elapsed=Math.max(0,now-this.started);
    const smooth=t => t*t*(3-2*t);
    const fadeIn=smooth(Math.min(1,elapsed/900));
    const fadeOut=this.ending ? 1-smooth(Math.min(1,Math.max(0,(now-this.waveEndedAt)/1600))) : 1;
    const gain=fadeIn*fadeOut;
    // Slow, slightly offset gestures; continue the same phase into the farewell.
    return [Math.sin(elapsed/1000*1.65)*gain,
      Math.sin(elapsed/1000*1.45+.65)*gain];
  }
  point(x, y, mouth, wave, blink=0, region=null, arms=[0,0]) {
    ...
    // Combine wrist waving and a gentle forearm bend in one continuous patch.
    ...
    if (region?.kind === 'arm') {
      ...
      if (region.side===0) {
        ...
        turn(91,224,arms[0]*.035+wave*.035,weight);
      } else {
        const weight=edge*smooth(221,257,sourceX);
        turn(246,310,-arms[1]*.032-wave*.045,weight);
      }
      return [x,y];
    }
  ...
  swayWaist(now) {
    if (!this.swayCtx || this.reduced.matches || (!this.speaking && !this.ending)) return;
    const smooth = t => t*t*(3-2*t);
    const elapsed = Math.max(0, now-this.started);
    const fadeIn = smooth(Math.min(1, elapsed/900));
    const fadeOut = this.ending
      ? 1-smooth(Math.min(1, Math.max(0, (now-this.waveEndedAt)/1600))) : 1;
    const sway = 5*Math.sin(elapsed/1000*1.5)*fadeIn*fadeOut;
    // Bend the composed portrait in horizontal strips so face and arm patches
    // follow the torso. Pin the top and bottom; the waist moves most.
    this.swayCtx.clearRect(0,0,320,480);
    this.swayCtx.drawImage(this.canvas,0,0);
    this.ctx.clearRect(0,0,320,480);
    for (let y=0; y<480; y++) {
      const weight = y<280 ? smooth(y/280) : 1-smooth((y-280)/200);
      this.ctx.drawImage(this.swayBuffer,0,y,320,1,sway*weight,y,320,1);
    }
  }
  draw(now) {
    ...
    if (now-this.lastFrame>=40) {
      ...
      // One smooth 1.6-second wave at each end, with gentle arm motion during speech.
      const phase=(now-(this.ending ? this.waveEndedAt : this.started))/1600;
      const wave=this.reduced.matches || phase<0 || phase>=1 ? 0
        : Math.sin(phase*Math.PI*2)*Math.sin(phase*Math.PI);
      this.render(mouth,wave,this.blinkTarget(now),this.armPose(now));
      this.swayWaist(now);
    }
    this.frame=requestAnimationFrame(this.draw);
  }
